...
- **Shell constraint:** `max-w-7xl` (1280px) until 2xl, then `max-w-[1600px]` (1536px+) centered via `mx-auto` — shared across Dashboard, Registro, and Locations. Single source of gutter truth: `px-4 sm:px-6 lg:px-8` and `py-8`. No outer padded wrapper (`p-4 md:p-8`) around the authenticated shell. One shared layout owns Navbar placement and gutters.
...
- **Navbar:** `{colors.surface}` on `{colors.foreground}`, `height 64px`, `border-b border-zinc-200`, no `backdrop-blur`, no `bg-background/50`. Sticky `top-0`. Internal max-width and gutters reuse shell tokens (`max-w-7xl 2xl:max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8`). Mark is the refined Bodega Técnica shelf-grid: 32×32, outer 16×16 frame at 8,8 with rx 1.5, 1.5px stroke, 2×2 bays one filled top-left, decorative SVG `aria-hidden`; wordmark “Bodega Técnica” + muted “ServiceFlow” (hidden below `sm`) carries accessible name, no duplicate `sr-only` filename, contrast 7.04:1 on `{colors.primary}`.
...
- **Owner:** one layout (e.g., `app/(app)/layout.tsx`) renders `<div class="min-h-screen bg-background"><Navbar /><main class="max-w-7xl 2xl:max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">` for all authenticated routes (`/dashboard`, `/locations`, `/service-events`). `/dashboard` must not re-wrap with `p-4 md:p-8` and a second `max-w-7xl` inside `ServicesDashboard`. Navbar inner row uses the same `max-w-7xl 2xl:max-w-[1600px]` cap so lockup, nav, and content share one edge.
