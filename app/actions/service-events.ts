...
import { serviceEventListBinding, applyBinding } from "@/lib/pocketbase-filter";
...
export async function getServiceEvents(params: {
	...
	try {
		...
		const binding = serviceEventListBinding({
			userId: user.id,
		...
		const result = await pb.collection("service_events").getList(page, limit, {
			filter,
			sort: "-changedAt",
		});
		...
		const rawServiceEvents = (result.items as any[]) ?? [];
		if (rawServiceEvents.length === 0) {
			return { data: [], total, page, limit };
		}

		// Enrich with service invoice/product/client and location names (best-effort)
		const serviceIds = Array.from(
			new Set(rawServiceEvents.map((l) => l.ServiceId).filter(Boolean) as string[]),
		);
		const locIds = Array.from(
			new Set(
				rawServiceEvents
					.flatMap((l) => [l.fromLocationId, l.toLocationId])
		...
		const data = rawServiceEvents.map((doc) => {
