...
describe("schema artifact", () => {
  it("declares four collections", () => {
    ...
    expect(names).toEqual(expect.arrayContaining(["users", "services", "locations", "location_logs"]));
  ...
  it("location_logs.userId required", () => {
    const { cols } = load();
    const ll = cols.find((c: any) => c.name === "location_logs");
    const f = (ll.fields ?? ll.schema ?? []).find((x: any) => x.name === "userId");
    expect(f.required).toBe(true);
  });
  it("tenant rule string on business collections", () => {
    const { cols } = load();
    for (const n of ["services", "locations", "location_logs"]) {
