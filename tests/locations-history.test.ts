...
const mockCollection = vi.fn((name: string) => {
	...
	if (name === "service_events") return { getList: mockLogsGetList };
	return {
		getList: mockLocationsGetList,
		getOne: mockLocationsGetOne,
		create: mockLocationsCreate,
...
describe("locations read WU3 — getLocations PocketBase tenant scope", () => {
	...
	it("authenticated binds userId = {:uid} via locationListBinding/applyBinding and calls PB collection locations", async () => {
		...
		expect(mockGetList).toHaveBeenCalledWith(
			1,
			expect.any(Number),
			expect.objectContaining({ filter: expect.any(String), sort: expect.any(String) }),
		);
	...
	it("page gate remains getAuthUser → redirect /login (auth-session spec)", async () => {
		const pageSrc = fs.readFileSync(
			path.join(process.cwd(), "app/(app)/locations/page.tsx"),
			"utf8",
		);
...
describe("locations write WU6b — create/update/toggle/delete", () => {
	beforeEach(() => {
		...
		mockLocationsDelete.mockResolvedValue({});
	...
	it("unauthenticated mutate writes nothing", async () => {
		...
		const {
			createLocation: c,
			updateLocation: u,
			toggleLocationActive: t,
			deleteLocation: d,
		} = await import("@/app/actions/locations");
		const fd = new FormData();
		fd.set("name", "Taller");
		expect(await c(null, fd)).toEqual({ error: "No autenticado" });
		const fd2 = new FormData();
		fd2.set("id", "loc1");
		fd2.set("name", "Taller");
		expect(await u(null, fd2)).toEqual({ error: "No autenticado" });
		expect(await t("loc1", false)).toEqual({ error: "No autenticado" });
		expect(await d("loc1", "Taller")).toEqual({ error: "No autenticado" });
		expect(mockCreatePocketBaseClient).not.toHaveBeenCalled();
		expect(mockLocationsCreate).not.toHaveBeenCalled();
	...
	it("create isActive true, server userId, address trimmed, native id omitted", async () => {
		...
		const fd = new FormData();
		fd.set("name", "  Taller Centro  ");
		fd.set("address", "  Calle 123  ");
		const r = await createLocation(null, fd);
		expect(r).toEqual(expect.objectContaining({ success: true }));
		const p = mockLocationsCreate.mock.calls[0][0] as Record<string, unknown>;
		expect(p.userId).toBe("u-owner-1");
		expect(p.isActive).toBe(true);
		expect(p.name).toBe("Taller Centro");
		expect(p.address).toBe("Calle 123");
		...
		const fd2 = new FormData();
		fd2.set("name", "Taller Sin Dir");
		fd2.set("address", "   ");
		const r2 = await createLocation(null, fd2);
		expect(r2).toEqual(expect.objectContaining({ success: true }));
		expect(
			(mockLocationsCreate.mock.calls[0][0] as Record<string, unknown>).address,
		).toBeUndefined();
	});
	it("duplicate exact rejected via normalizeString and bound userId only", async () => {
		mockLocationsGetList.mockResolvedValue({
			items: [{ id: "loc1", name: "Taller Centro", userId: "u-owner-1" }],
			totalItems: 1,
		});
		const { createLocation } = await import("@/app/actions/locations");
		const fd = new FormData();
		fd.set("name", "Taller Centro");
		const r = await createLocation(null, fd);
		expect(r.error).toMatch(/Ya existe.*Sede.*nombre/i);
		expect(mockLocationsCreate).not.toHaveBeenCalled();
		expect(mockFilter).toHaveBeenCalled();
		const [t, p] =
			mockFilter.mock.calls.find(([x]) => (x as string).includes("userId")) ??
			mockFilter.mock.calls[0];
		expect(t as string).toContain("userId = {:uid}");
		expect((p as Record<string, unknown>).uid).toBe("u-owner-1");
		expect(t as string).not.toContain("Taller Centro");
	});
	it("update keeps own name without duplicate error", async () => {
		...
		mockLocationsUpdate.mockResolvedValue({ id: "loc1", name: "Taller Centro" });
		const { updateLocation } = await import("@/app/actions/locations");
		const fd = new FormData();
		fd.set("id", "loc1");
		fd.set("name", "Taller Centro");
		fd.set("address", "Nueva Dir");
		const r = await updateLocation(null, fd);
		expect(r).toEqual(expect.objectContaining({ success: true }));
		expect(mockLocationsUpdate).toHaveBeenCalledTimes(1);
	});
	it("peer mutate fails with not-found/unauthorized and no mutation", async () => {
		mockGetAuthUser.mockResolvedValue({ id: "user-B", email: "b@test.com", name: "B" });
		mockLocationsGetOne.mockResolvedValue({
			id: "loc1",
			name: "Taller",
			userId: "user-A",
		...
		fd.set("id", "loc1");
		fd.set("name", "Hack");
		expect((await u(null, fd)).error).toMatch(/no encontrada|No autorizado/i);
		expect(mockLocationsUpdate).not.toHaveBeenCalled();
		expect((await t("loc1", false)).error).toMatch(/No autorizado|no encontrada/i);
		expect(mockLocationsUpdate).not.toHaveBeenCalled();
		mockServicesGetList.mockResolvedValue({ items: [], totalItems: 0 });
		mockLogsGetList.mockResolvedValue({ items: [], totalItems: 0 });
		expect((await d("loc1", "Taller")).error).toMatch(/no encontrada|No autorizado/i);
		expect(mockLocationsDelete).not.toHaveBeenCalled();
	});
	it("delete blocked with service or log, same Spanish guard; unused deletes", async () => {
		...
		mockLogsGetList.mockResolvedValue({ items: [], totalItems: 0 });
		mockLocationsDelete.mockResolvedValue({});
		expect(await deleteLocation("loc1", "Taller")).toEqual(
			expect.objectContaining({ success: true }),
		);
		expect(mockLocationsDelete).toHaveBeenCalledWith("loc1");
	});
	...
	it("accent-insensitive duplicate and cross-tenant allowed, toggle keeps history", async () => {
		mockLocationsGetList.mockResolvedValue({
			items: [{ id: "loc1", name: "Ñuñoa", userId: "u-owner-1" }],
			totalItems: 1,
		});
		const { createLocation: c, toggleLocationActive: t } = await import("@/app/actions/locations");
		const fd = new FormData();
		fd.set("name", "Nunoa");
		expect((await c(null, fd)).error).toMatch(/Ya existe.*Sede.*nombre/i);
		expect(mockLocationsCreate).not.toHaveBeenCalled();
		mockGetAuthUser.mockResolvedValue({ id: "user-B", email: "b@test.com", name: "B" });
		mockLocationsGetList.mockResolvedValue({ items: [], totalItems: 0 });
		mockLocationsCreate.mockResolvedValue({
		...
		const fd2 = new FormData();
		fd2.set("name", "Taller Centro");
		expect(await c(null, fd2)).toEqual(expect.objectContaining({ success: true }));
		expect(mockLocationsCreate).toHaveBeenCalled();
		vi.clearAllMocks();
		mockGetAuthUser.mockResolvedValue({ id: "u-owner-1", email: "owner@test.com", name: "Owner" });
		mockLocationsGetOne.mockResolvedValue({
			id: "loc1",
		...
		mockLocationsUpdate.mockResolvedValue({ id: "loc1", isActive: false });
		expect(await t("loc1", false)).toEqual(expect.objectContaining({ success: true }));
...
describe("history read WU6d — getServiceEvents PocketBase tenant-bound envelope", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		mockGetAuthUser.mockReset();
		mockLocationsGetList.mockReset();
	...
	it("unauthenticated → { error: 'No autenticado' } without data and no PB call", async () => {
		mockGetAuthUser.mockResolvedValue(null);
		const { getServiceEvents } = await import("@/app/actions/service-events");
		const res = await getServiceEvents({ page: 1, limit: 20 });
		expect(res).toEqual({ error: "No autenticado" });
	...
	it("authenticated envelope binds userId = {:uid}, sort -changedAt, pagination, peer isolation, total", async () => {
		...
		const { getServiceEvents } = await import("@/app/actions/service-events");
		const res = await getServiceEvents({ page: 2, limit: 5 });
		expect(res).toEqual(
			expect.objectContaining({ data: expect.any(Array), total: 1, page: 2, limit: 5 }),
		);
		...
		expect(mockFilter).toHaveBeenCalled();
		const [template, params] = mockFilter.mock.calls.find(([t]) =>
			(t as string).includes("userId"),
		...
		expect(mockLogsGetList).toHaveBeenCalledWith(
			2,
			5,
			expect.objectContaining({ sort: "-changedAt", filter: expect.any(String) }),
		);
		vi.clearAllMocks();
		...
		mockCollection.mockClear();
		mockCreatePocketBaseClient.mockClear();
		const { getServiceEvents: getB } = await import("@/app/actions/service-events");
		await getB({ page: 1, limit: 20 });
		const [templateB, paramsB] = mockFilter.mock.calls.find(([t]) =>
			(t as string).includes("userId"),
		) as [string, Record<string, unknown>];
	...
	it("from-or-to locationId filter, date bounds, empty list, id mapping (id not $id)", async () => {
		...
		mockLogsGetList.mockResolvedValue({
			items: [
				...
				},
				{
					...
					fromLocationId: "locY",
					toLocationId: "locZ",
					changedAt: "2024-06-02T00:00:00.000Z",
				},
			],
			totalItems: 2,
		});
		const { getServiceEvents } = await import("@/app/actions/service-events");
		const res = await getServiceEvents({
			locationId: "locY",
		...
		const [fullTemplate, fullParams] = mockFilter.mock.calls.find(([t]) =>
			(t as string).includes("userId"),
		) as [string, Record<string, unknown>];
		...
		mockLogsGetList.mockResolvedValue({ items: [], totalItems: 0 });
		const empty = await getServiceEvents({ page: 1, limit: 20 });
		...
		const withDefaults = await getServiceEvents({});
		expect(withDefaults.page).toBe(1);
		expect(withDefaults.limit).toBe(20);
	});

	it("source uses PB serviceEventListBinding/applyBinding, keeps page gates, generic error on throw", async () => {
		const src = fs.readFileSync(path.join(process.cwd(), "app/actions/service-events.ts"), "utf8");
		expect(src).toContain("createPocketBaseClient");
		expect(src).toContain("serviceEventListBinding");
		expect(src).toContain("applyBinding");
		expect(src).toContain('collection("service_events")');
		expect(src).toContain("getList");
		expect(src).toContain("getAuthUser");
		expect(src).toContain("No autenticado");
		const logsPage = fs.readFileSync(
			path.join(process.cwd(), "app/(app)/service-events/page.tsx"),
			"utf8",
		);
		...
		const { getServiceEvents } = await import("@/app/actions/service-events");
		const err = await getServiceEvents({ page: 1, limit: 20 });
