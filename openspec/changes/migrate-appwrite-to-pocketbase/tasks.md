...
- [x] (A) Dev candidates — may start now (no prod gate): open draft/no-merge PRs WU10a (janitor/session/tests) and WU10b (Appwrite client/setup/deps/lock + final proof) chained from `…-09-hygiene`; acceptance (section 17 B) only gates final merge/deploy, not code edits or local/CI. <!-- sdd-owner: implementation -->
